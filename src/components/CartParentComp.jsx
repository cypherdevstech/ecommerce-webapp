...
const CartParentComp = ({ productId }) => {
  ...
  console.log('product from cart', cartItems)
  return (
    <aside className='w-full max-w-[30%] bg-white p-5 rounded-2xl h-auto min-h-[250px]'>
      <div className='w-full flex justify-start items-center'>
        <h2 className='text-base md:text-2xl text-[#c73a0f] font-semibold'>Your Cart ({cartItems?.length})</h2>
      </div>
      {/* cart content shows here */}
      {
        cartItems?.length > 0 ? (<div className='h-atuo hidden'></div>) : (<div className='mx-auto flex flex-col gap-3 w-full'>
